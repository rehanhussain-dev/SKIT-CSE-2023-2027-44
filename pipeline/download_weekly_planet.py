...
def poll_and_transfer_order(order_id, drive_service, drive_folder_id, poll_interval=15):
    ...
    while True:
        ...
        if state == "success":
            ...
            print(
                f"Downloading and transferring {len(results)} outputs to Google Drive..."
            )
            for item in results:
            ...
            raise SystemExit(
                f"Planet order failed: {state}. Details: {data.get('error')}"
            )
...
def main():
    print(
        f"Initializing Google Drive service for target folder '{DRIVE_TARGET_FOLDER}'..."
    )
